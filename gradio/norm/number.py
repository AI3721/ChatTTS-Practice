...
RE_FRAC = re.compile(r'(-?)(\d+)/(\d+)') # 分数
RE_INTEGER = re.compile(r'(-)' r'(\d+)') # 整数
RE_DEFAULT_NUM = re.compile(r'\d{3}\d*') # 编号
RE_PERCENTAGE = re.compile(r'(-?)(\d+(\.\d+)?)%') # 百分数
RE_NUMBER = re.compile(r'(-?)((\d+)(\.\d+)?)' r'|(\.(\d+))') # 实数
RE_DECIMAL_NUM = re.compile(r'(-?)((\d+)(\.\d+))' r'|(\.(\d+))') # 小数
RE_POSITIVE_QUANTIFIERS = re.compile(r"(\d+)([多余几\+])?" + COM_QUANTIFIERS) # 余数
RE_RANGE = re.compile(r'((-?)((\d+)(\.\d+)?)|(\.(\d+)))[-~]((-?)((\d+)(\.\d+)?)|(\.(\d+)))') # 范围
